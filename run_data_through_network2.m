function [yhat, mappedX, M] = run_data_through_network2(network, X, noise)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    for i=1:length(network) - 1
        if i ~= 1
            %mappedX = [max(mappedX * [network{i}.W .* (1 - noise(2)); network{i}.bias_upW], 0) ones(n, 1)];
            ...
            acts = zeros(n, size(network{i+1}.W, 1) + 1);
            presyn = mappedX * [network{i}.W .* (1 - noise(2)); network{i}.bias_upW];
            for j = 1:n
                tmp = reshape(presyn(j,:), [5 length(presyn(j,:)) ./ 5]);
                acts(j,:) = [max(tmp) 1];
            end
            mappedX = acts;
            ...
            acts = zeros(n, size(network{i+1}.W, 1) + 1);
            presyn = mappedX * [network{i}.W .* (1 - noise(1)); network{i}.bias_upW];
            for j = 1:n
                tmp = reshape(presyn(j,:), [5 length(presyn(j,:)) ./ 5]);
                acts(j,:) = [max(tmp) 1];
            end
