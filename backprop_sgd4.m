function [network, results] = backprop_sgd4(network, train, train_labels, test, test_L, noise)
        ...
        for batch=1:n:numpts
            ...
            for i=1:no_layers
                if i ~= no_layers
                    ...
                    layerin{i+1} = cell(sx, 1);
                    acts{i+1} = zeros(sx, size(network{i+1}.W, 1) + 1);
                    presyn = acts{i} * [network{i}.W; network{i}.bias_upW];
                    for j = 1:sx
                        layerin{i+1}{j} = reshape(presyn(j,:), [5 length(presyn(j,:)) ./ 5]);
                        acts{i+1}(j,:) = [max(layerin{i+1}{j}) 1];
                    end
            ...
            for i=no_layers:-1:1
                ...
                if i > 1
                    ...
                    Ix = Ix(:,1:end - 1);
                    Ix = kron(Ix, ones(1, 5));
                    dacts{i} = zeros(size(Ix));
                    for j = 1:sx
                        g = bsxfun(@eq, layerin{i}{j}, max(layerin{i}{j}));
                        dacts{i}(j,:) = g(:)';
                    end
                    Ix = Ix .* dacts{i};
            ...
            %    disp(['Error: ' num2str(sum(test_L ~= yhat) ./ length(yhat))]);
            %end
            loop = loop + 1;
        end
